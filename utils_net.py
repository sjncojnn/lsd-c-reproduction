...
class VGG4MNIST(nn.Module):
    def __init__(self, num_classes=10):
        super(VGG4MNIST, self).__init__()
        ...
        self.fc = nn.Linear(128 * 7 * 7, 512)   # ← đổi tên để dễ quản lý
        self.classifier = nn.Linear(512, num_classes)

    def forward(self, x):
        x = self.features(x)
        x = x.view(x.size(0), -1)
        feat = F.relu(self.fc(x))               # ← feature 512-dim
        out = self.classifier(feat)             # ← output 10-dim
        return out, feat
